...
Function Provision-VMsForLisa($allVMData, $installPackagesOnRoleNames)
{
	if($user -ne "root") {
		...
		foreach ( $vmData in $allVMData )
		{
			...
			Copy-RemoteFiles -uploadTo $vmData.PublicIP -port $vmData.SSHPort -files ".\Testscripts\Linux\utils.sh,.\Testscripts\Linux\enableRoot.sh,.\Testscripts\Linux\enablePasswordLessRoot.sh" -username $user -password $password -upload -uploadToFolder "/home/$user"
			$Null = Run-LinuxCmd -ip $vmData.PublicIP -port $vmData.SSHPort -username $user -password $password -command "chmod +x *.sh" -runAsSudo -ExecuteFolder "/home/$user"
			$rootPasswordSet = Run-LinuxCmd -ip $vmData.PublicIP -port $vmData.SSHPort `
				-username $user -password $password -runAsSudo `
				-command ("./enableRoot.sh -password {0}" -f @($password.Replace('"',''))) -ExecuteFolder "/home/$user"
			...
			$Null = Run-LinuxCmd -ip $vmData.PublicIP -port $vmData.SSHPort -username "root" -password $password -command "cp -ar /home/$user/*.sh ." -ExecuteFolder "/root"
			if ( $keysGenerated )
			{
				Copy-RemoteFiles -uploadTo $vmData.PublicIP -port $vmData.SSHPort -files "$LogDir\sshFix.tar" -username "root" -password $password -upload -uploadToFolder "/root"
				$keyCopyOut = Run-LinuxCmd -ip $vmData.PublicIP -port $vmData.SSHPort -username "root" -password $password -command "./enablePasswordLessRoot.sh" -ExecuteFolder "/root"
				Write-LogInfo $keyCopyOut
				if ( $keyCopyOut -imatch "KEY_COPIED_SUCCESSFULLY" )
				{
					$keysGenerated = $true
					Write-LogInfo "SSH keys copied to $($vmData.RoleName)"
					$md5sumCopy = Run-LinuxCmd -ip $vmData.PublicIP -port $vmData.SSHPort -username "root" -password $password -command "md5sum .ssh/id_rsa" -ExecuteFolder "/root"
			...
			{
				$Null = Run-LinuxCmd -ip $vmData.PublicIP -port $vmData.SSHPort -username "root" -password $password -command "rm -rf /root/sshFix*" -ExecuteFolder "/root"
				$keyGenOut = Run-LinuxCmd -ip $vmData.PublicIP -port $vmData.SSHPort -username "root" -password $password -command "./enablePasswordLessRoot.sh" -ExecuteFolder "/root"
				Write-LogInfo $keyGenOut
				if ( $keyGenOut -imatch "KEY_GENERATED_SUCCESSFULLY" )
				{
					$keysGenerated = $true
					Write-LogInfo "SSH keys generated in $($vmData.RoleName)"
					Copy-RemoteFiles -download -downloadFrom $vmData.PublicIP -port $vmData.SSHPort  -files "./sshFix.tar" -username "root" -password $password -downloadTo $LogDir -downloadFromFolder "/root"
					$md5sumGen = Run-LinuxCmd -ip $vmData.PublicIP -port $vmData.SSHPort -username "root" -password $password -command "md5sum .ssh/id_rsa" -ExecuteFolder "/root"
				}
				else
				{
		...
		foreach ( $vmData in $allVMData )
		{
			if ( $installPackagesOnRoleNames )
			{
				if ( $installPackagesOnRoleNames -imatch $vmData.RoleName )
				{
					...
					$jobID = Run-LinuxCmd -ip $vmData.PublicIP -port $vmData.SSHPort -username "root" -password $password -command "./$scriptName" -RunInBackground -ExecuteFolder "/root"
		...
		while ($packageInstallJobsRunning)
		{
			...
			foreach ( $job in $packageInstallJobs )
			{
				...
				{
					$currentStatus = Run-LinuxCmd -ip $job.PublicIP -port $job.SSHPort -username "root" -password $password -command "tail -n 1 ./provisionLinux.log" -ExecuteFolder "/root"
					Write-LogInfo "Package Installation Status for $($job.RoleName) : $currentStatus"
					$packageInstallJobsRunning = $true
				}
				else
				{
					Copy-RemoteFiles -download -downloadFrom $job.PublicIP -port $job.SSHPort -files "./provisionLinux.log" `
						-username "root" -password $password -downloadTo $LogDir -downloadFromFolder "/root"
		...
		!($CustomKernel.EndsWith(".rpm")) -and !($CustomKernel.EndsWith(".tar.gz")) -and !($CustomKernel.EndsWith(".tar")) ) {
			...
			foreach ( $vmData in $allVMData ) {
				...
				$jobID = Run-LinuxCmd -ip $vmData.PublicIP -port $vmData.SSHPort -username $user `
					-password $password -command "./$scriptName -CustomKernel '$CustomKernelLabel' -logFolder /tmp" `
...
Function Remove-AllFilesFromHomeDirectory($AllDeployedVMs, $User, $Password)
{
	foreach ($DeployedVM in $AllDeployedVMs)
	{
		...
		try
		{
			...
			Write-LogInfo "All files removed from /tmp successfully. VM IP : $testIP PORT : $testPort"
		}
		catch
		{
			$ErrorMessage =  $_.Exception.Message
