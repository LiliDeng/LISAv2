...
"@
		...
		Copy-RemoteFiles -uploadTo $AllVmData.PublicIP -port $AllVmData.SSHPort `
			-files "$constantsFile,$LogDir\StartBuildInstallRdmaCore.sh" -username $user -password $password -upload
		...
		$testJob = Run-LinuxCmd -ip $AllVmData.PublicIP -port $AllVmData.SSHPort `
			-username $user -password $password -command "bash StartBuildInstallRdmaCore.sh" -RunInBackground -runAsSudo
		#endregion

		#region MONITOR BUILD INSTALL RDMA CORE
		while ((Get-Job -Id $testJob).State -eq "Running") {
			$currentStatus = Run-LinuxCmd -ip $AllVmData.PublicIP -port $AllVmData.SSHPort `
				-username $user -password $password -command "tail -2 /root/buildInstallRdmaCoreLogs.txt | head -1" -runAsSudo
...
"@
		...
		Copy-RemoteFiles -uploadTo $AllVmData.PublicIP -port $AllVmData.SSHPort `
			-files "$constantsFile,$LogDir\StartDpdkPrimarySecondarySetup.sh" -username $user -password $password -upload

		Run-LinuxCmd -ip $AllVmData.PublicIP -port $AllVmData.SSHPort `
			-username $user -password $password -command "chmod +x *.sh; cp * /root/" -runAsSudo | Out-Null
		$testJob = Run-LinuxCmd -ip $AllVmData.PublicIP -port $AllVmData.SSHPort `
			-username $user -password $password -command "bash StartDpdkPrimarySecondarySetup.sh" -RunInBackground -runAsSudo
		#endregion
...
"@
		...
		Copy-RemoteFiles -uploadTo $AllVmData.PublicIP -port $AllVmData.SSHPort `
			-files "$constantsFile,$LogDir\StartDpdkPrimarySecondaryVerify.sh" -username $user -password $password -upload

		Run-LinuxCmd -ip $AllVmData.PublicIP -port $AllVmData.SSHPort `
			-username $user -password $password -command "chmod +x *.sh; cp * /root/" -runAsSudo | Out-Null
		$testJob = Run-LinuxCmd -ip $AllVmData.PublicIP -port $AllVmData.SSHPort `
			-username $user -password $password -command "bash StartDpdkPrimarySecondaryVerify.sh" -RunInBackground -runAsSudo
		#endregion

		#region MONITOR INSTALL CONFIGURE DPDK PRIMARY/SECONDARY
		while ((Get-Job -Id $testJob).State -eq "Running") {
			$currentStatus = Run-LinuxCmd -ip $AllVmData.PublicIP -port $AllVmData.SSHPort `
				-username $user -password $password -command "tail -2 /root/dpdkVerifyPrimarySecondaryLogs.txt | head -1" -runAsSudo
			Write-LogInfo "Current Test Status : $currentStatus"
			Wait-Time -seconds 20
		}
		$dpdkStatus = Run-LinuxCmd -ip $AllVmData.PublicIP -port $AllVmData.SSHPort `
