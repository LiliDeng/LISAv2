...
function Main {
    ...
    $usedSpaceVM = Run-LinuxCmd -username  $VMUserName -password $VMPassword -ip $ipv4vm1 -port $VMPort -command "df -B1 `$(pwd) |  awk '{print `$3}' | tail -1" -RunAsSudo
    ...
    Run-LinuxCmd -username $VMUserName -password $VMPassword -ip $ipv4vm1 -port $VMPort -command "nohup dd if=/dev/urandom of=`$(pwd)/data2 bs=1M count=$ddFileSize" -RunInBackGround -RunAsSudo
    Start-Sleep 90
