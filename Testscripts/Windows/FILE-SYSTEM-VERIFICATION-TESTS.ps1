...
function Main {
    ...
    try {
        ...
        Run-LinuxCmd -ip $allVMData.PublicIP -port $allVMData.SSHPort -username $user `
            -password $password -command "bash xfstesting.sh" -runAsSudo -RunInBackground | Out-Null
        ...
        while ($sw.elapsed -lt $timeout) {
            ...
            if ($tcpTestResult) {
                ...
                -username $user -password $password "cat state.txt" -runAsSudo
        ...
        Run-LinuxCmd -ip $allVMData.PublicIP -port $allVMData.SSHPort -username $user `
            -password $password -command "chown -R ${user}:${user} /root;cp /root/* ." -ignoreLinuxExitCode:$true -runAsSudo | Out-Null
